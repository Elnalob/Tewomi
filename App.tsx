  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors duration-200">
      {showInstallBanner && deferredPrompt && (
        <div className="bg-indigo-600 text-white p-4 flex items-center justify-between sticky top-0 z-[60] shadow-lg animate-in fade-in slide-in-from-top duration-500">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-medium">Install Tèwómí for a better experience</p>
          </div>
      ...
      <header className="px-6 h-20 flex items-center justify-between max-w-7xl mx-auto w-full">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-slate-900 dark:bg-indigo-600 rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg">T</div>
          <span className="font-black text-2xl tracking-tighter text-slate-900 dark:text-slate-50">Tèwómí</span>
        </div>
      ...
      <footer className="p-12 text-center text-slate-400 dark:text-slate-600 text-[10px] font-black uppercase tracking-[0.2em]">
        Tèwómí Invoicer &bull; Powered by solo-hustle &bull; Nigeria 🇳🇬
      </footer>
    </div>
  );
};
  ...
  return (
    <HashRouter>
      <Routes>
        ...
        <Route path="/invoice/:id" element={<div className="bg-slate-50 dark:bg-slate-950 min-h-screen p-8 transition-colors"><InvoiceDetail /></div>} />
